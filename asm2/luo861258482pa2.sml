...
fun find23 myComp  EmptyTree myValue= NONE
	...
	if myComp(myValue,leftval) = 0
	then
		SOME(leftval)

	else
		if myComp(myValue,rightval) = 0
		then
			SOME(rightval)
		else
...
fun addTree myComp myValue  EmptyTree =
	...
	then
		...
		then
			...
			FullNode(rootval,myValue,left,EmptyTree,right)
		...
		in
			...
			then
				...
				then
					...
					in
						SingleNode(leftval,newNode,newRight)
					end
			else
					FullNode(leftval,rightval,newNode,center,right)
		end
	...
	then
		...
		then
			...
			then
				...
				if myComp(myValue, #1 rightval) = ~1
				then
